...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    //condition that the words are not in the dictionary
    if (dict.find(a) == dict.end() || dict.find(b) == dict.end()) return false;

    if (dict.at(a).size() != dict.at(b).size()) return false;

    for (size_t i = 0; i < dict.at(a).size(); i++) {
        if (dict.at(b).at(i) != dict.at(a).at(i)) return false;
    }
