...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
     ...
     */
    if (first_larger_idx < subroot->elements.size() && key == subroot->elements[first_larger_idx]) {
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if (first_larger_idx < subroot->elements.size() && subroot->elements[first_larger_idx] == pair) return;
    if (subroot->is_leaf){
        subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
    } 
    else {
      BTreeNode* newchild = subroot->children[first_larger_idx];
      insert(newchild, pair);
      if (subroot->children[first_larger_idx]->elements.size() > order-1) {
