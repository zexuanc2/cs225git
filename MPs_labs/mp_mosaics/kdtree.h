...
class KDTree
{
  ...
  private:
    ...
    /**
     * the first is the helper function for the following quickselect algorithm.
     * the second is actually the function implementing it.
     */ 
    unsigned quickSelect_position(vector<Point<Dim>>& list, int dimension, unsigned left, unsigned right, unsigned pivotIndex);
    Point<Dim>& quickSelect(vector<Point<Dim>>& list, int dimension, unsigned left, unsigned right, unsigned i);

    /**
     * helper function of the kdtree constructor.
     */
    KDTreeNode* ctorHelper(vector<Point<Dim>>& points_, int dimension, unsigned left, unsigned right);

    /**
     * helper function of the findNearestNeighbor.
     */
